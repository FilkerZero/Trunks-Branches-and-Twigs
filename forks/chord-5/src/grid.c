...
extern int new_define_format;    /* Chord Pro Manager support */
extern int warning_level;        /* how much warning do we want? */
extern int auto_adjust;          /* Auto-adjust some chord definitions */
/*--------------------------------------------------------------------------------*/
...
int do_define_chord()
{
	...
	/* CHORD is OK */
	/* auto adjust chords to reduce the notation problems. */
	if (hardtoplay && auto_adjust)
	{
	    int low = LONG_FINGERS;
	    hardtoplay = 0;
	    for (n = 0; n < 6; ++n)
	    {
		if ((dot_array[n] != FRET_X) && (dot_array[n] != 0))
		{
		    if (dot_array[n] < low)
		    {
			low = dot_array[n];
		...
		if (warning_level > 1)
		{
		    ...
		    sprintf(mesg,
			    "Adjusted grid for chord \"%s\" to {define: %s base_fret %d frets",
			    chord_name, chord_name, fret_displ);
			...
			{
			    sprintf(temp_buf, " %d", dot_array[n]);
			    strcat(mesg, temp_buf);
			}
		    }
		    strcat(mesg, "}");
		    error(mesg);
...
/*--------------------------------------------------------------------------------*/
/* support the Chord Pro Manager version of chord definition
 * {chord: chrdname 6 5 4 3 2 1 offset} where x, rather than -, is used
 * for muted strings.
 */
int do_chord_define()
{
	...
	strcpy(temp_str, toupper_str(strtok(NULL, DELIM_STR)));
#if 0
	fprintf(stderr, "chord debug: String %i: %s\n", n + 1, temp_str);
#endif
	if (temp_str!= NULL) {
	    if (!strcmp(temp_str, FRET_NONE_STR) ||
		!strcmp(temp_str, FRET_X_STR))
    ...
    temp_str= strtok(NULL, DELIM_STR);
    if (temp_str== NULL) {
	error("syntax error in chord definition: no offset value");
	return(0);
    }
	...
	if (low > 2)
	{
	    ...
	    if (warning_level > 1)
	    {
		sprintf(mesg, "Adjusted grid for chord \"%s\" to {chord: %s ",
    ...
    if (hardtoplay && (warning_level > 0))
    {
	sprintf(mesg, "You'll need long fingers to play \"%s\" this way!", 
		chord_name); 
	error (mesg);
    }
