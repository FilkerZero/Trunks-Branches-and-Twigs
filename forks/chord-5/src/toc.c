...
extern int vpos;
extern font_info font_table[];
extern page_info page_desc;
...
void build_ps_toc()
	...
	number_all= FALSE;
	song_pages= 0;
	set_text_font (font_table[FONT_TEXT].size+10);
	use_text_font();
	printf ("(");
	ps_puts (&title1);
	...
	while (toc_ptr != NULL)
		...
		/* title */
		if ( vpos < page_desc.bottom + 3 * font_table[FONT_TEXT].size)
			{
			advance (vpos);
			song_pages= 0;
			}
		advance(font_table[FONT_TEXT].size + 8);
